...
![](countries.png)
Shiny application deployed to visualize the Tiers of countries
...
```{r data_reactiveinput, warning=FALSE, echo = FALSE, message=FALSE, results='hold'}

date_list <- unique(data_geom$first_visit)

shiny::sliderInput("first_visit_var", "", animate = T,
 min = min(date_list), max = max(date_list), value = 2022)

```


```{r data_render, warning=FALSE, message=FALSE, results='hold'}

renderLeaflet({
  data_geom %>%
  st_as_sf() %>% 
  filter(first_visit <= input$first_visit_var) %>%
  leaflet() %>%
